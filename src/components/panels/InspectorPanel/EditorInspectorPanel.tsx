/**
 * Editor Inspector Panel - Uses nova-ecs-editor package for component metadata
 * 编辑器检查器面板 - 使用nova-ecs-editor包获取组件元数据
 * 
  ...
  return (
    <Card
      ...
      style={{
        ...
        borderColor: enabled ? '#303030' : '#404040'
      }}
      title={
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
...
/**
 * Main Editor Inspector Panel component
 * 主编辑器检查器面板组件
 */
export interface EditorInspectorPanelProps {
  style?: React.CSSProperties;
  className?: string;
}

export const EditorInspectorPanel: React.FC<EditorInspectorPanelProps> = ({
  style,
  className
}) => {
    ...
    >
      {!selectedEntity ? (
        ...
        <>
          ...
          >
            <Select
              placeholder="Select component type 选择组件类型"
              style={{ width: '100%' }}
              size="small"
              onSelect={handleAddComponent}
            >
              {getAvailableComponentTypes().map(({ componentType, registration }: any) => (
                <Option key={componentType.name} value={componentType.name}>
                  <Space>
                    <span>{registration.metadata.icon || '📦'}</span>
                    <span>{registration.metadata.displayName}</span>
                    {registration.metadata.category && (
                      <Tag color="blue">{registration.metadata.category}</Tag>
                    )}
                  </Space>
                </Option>
              ))}
